...
(function (global) {
  ...
  /**
   * ขอบล่างจริงของข้อความที่เพิ่งพิมพ์ไป (รวมสระล่าง/วรรณยุกต์ล่าง)
   *
   * ใช้กันไม่ให้ข้อความของสมาชิกไปทับลายเซ็นประธานชมรม
   * คำนวณจากผลจริงที่ fitText คืนมา ไม่ใช่ตัวเลขที่เดาไว้ล่วงหน้า
   * เพราะจำนวนบรรทัดและขนาดที่ย่อลงขึ้นกับข้อความของสมาชิกแต่ละคน
   */
  function blockBottom(res, y) {
    if (!res || !res.lines || !res.lines.length) return y;
    var lh = res.height / res.lines.length;
    return y + (res.lines.length - 1) * lh + res.size * 0.3528 * 0.3;
  }
   ...
   *   37.2 - 42.4   ลายมือชื่อเจ้าของบัตร อยู่ใต้รูปถ่ายในคอลัมน์เดียวกัน
   *   16.2 - ~35.6  คอลัมน์ขวา ชื่อ-นามสกุล ตำแหน่ง และหน่วยงาน
   *   27.0 - 46.4   ตราประทับหมึกแดง ขยายขึ้นถึงกึ่งกลางบัตร วาดเป็นภาพเบื้องหลัง
   *   ~33 - 45.8    ช่องลงนามประธานชมรม (ความสูงยืดหยุ่นตามที่ว่างจริง)
   *   46.8 - 54.0   แถบท้ายบัตร รหัสสมาชิกและวันหมดอายุ
   *
   * กติกาสามข้อของครึ่งขวาที่ห้ามแก้ให้เสีย
   *   1. ตำแหน่งและชื่อหน่วยงานของสมาชิก ใช้ขนาด 80% ของชื่อ-นามสกุลสมาชิก
   *      คิดจากขนาดที่ย่อจริงของชื่อ ไม่ใช่ขนาดตั้งต้น
   *   2. ตราประทับเป็นภาพเบื้องหลัง วาดก่อนทุกข้อความของครึ่งขวา
   *   3. ข้อความของสมาชิกต้องไม่ทับลายเซ็นประธานชมรม
   *      บังคับด้วยการคำนวณขอบล่างจริงของข้อความ แล้วดันขอบบนของช่องลายเซ็นลงมา
   *      ไม่ใช่การกะระยะไว้ล่วงหน้า เพราะข้อความของแต่ละคนยาวไม่เท่ากัน
   *
   * บรรทัดของช่องลงนามประธานชมรมยึดจากล่างขึ้นบน
   * ถ้าไม่ได้กรอกตำแหน่งประธานชมรม บรรทัดชื่อจะเลื่อนลงมาชิดบรรทัดชื่อชมรม
   * ไม่เว้นช่องว่างของบรรทัดที่ไม่มีไว้
   */
  function drawFront(d, data, ox, oy) {
    ...
    /* ---- ตราประทับหมึกแดง: ภาพเบื้องหลังของครึ่งขวา ---- */
    /*
     * วาดก่อนทุกอย่างในครึ่งขวา ตราจึงอยู่ด้านหลังข้อความและลายเซ็นทุกบรรทัด
     * ขอบบนอยู่ที่กึ่งกลางบัตรพอดี ขอบล่างชิดแถบท้ายบัตร
     * ห้ามย้ายบรรทัดนี้ไปไว้หลังการพิมพ์ข้อความ ตราจะทับข้อความจนอ่านไม่ออก
     */
    var gw = 36;
    var gx = ox + CARD_W - 4.6 - gw;
    var gcx = gx + gw / 2;
    var stampTop = oy + CARD_H / 2;
    var stampBottom = footTop - 0.4;
    drawStamp(d, gcx, (stampTop + stampBottom) / 2, stampBottom - stampTop);
    ...
    fitText(d, "บัตรสมาชิก / MEMBER CARD", {
      x: tx, y: oy + 16.2, maxWidth: tw, size: 4.5, minSize: 3.4, color: COLOR.ink3
    });

    // ชื่อ-นามสกุล: ต้องพอดีบัตร ไม่ตัดข้อความ
    var nameRes = fitText(d, data.fullName || "", {
      x: tx, y: oy + 21.2, maxWidth: tw, size: 11.2, minSize: 6.4, style: "bold", color: COLOR.ink
    });

    /*
     * ตำแหน่งและชื่อหน่วยงานใช้ขนาด 80% ของชื่อ-นามสกุลสมาชิก
     * คิดจากขนาดที่ fitText ย่อจริง ไม่ใช่ขนาดตั้งต้น 11.2
     * ชื่อยาวจะถูกย่อลง ตำแหน่งและหน่วยงานจึงย่อตามให้ได้สัดส่วนเดิมเสมอ
     */
    var subSize = Math.max(4.6, (nameRes.size || 11.2) * 0.8);
    var subLead = Math.max(3.2, subSize * 0.3528 * 1.06);
    ...
    if (nameEn) {
      yRow = oy + 24.8;
      fitText(d, nameEn, {
        x: tx, y: yRow, maxWidth: tw, size: 4.8, minSize: 3.8, color: COLOR.ink2
      });
    }

    // ตำแหน่ง และ ชื่อหน่วยงาน: คนละบรรทัดเสมอ บรรทัดที่ไม่มีข้อมูลไม่กินที่
    var memberBottom = yRow;
    if (data.position) {
      yRow += subLead;
      memberBottom = blockBottom(fitText(d, data.position, {
        x: tx, y: yRow, maxWidth: tw, size: subSize, minSize: 4.2, color: COLOR.ink2
      }), yRow);
    }
    if (data.org) {
      yRow += subLead;
      memberBottom = blockBottom(fitText(d, data.org, {
        x: tx, y: yRow, maxWidth: tw, maxLines: 2,
        size: subSize, minSize: 4, lineFactor: 1.02, color: COLOR.brandDark
      }), yRow);
    }

    /* ---- ช่องลงนามประธานชมรม: ลายเซ็น ชื่อในวงเล็บ ตำแหน่ง และบรรทัดประธานชมรม ---- */
    /*
     * เรียงจากล่างขึ้นบน บรรทัดชื่อชมรมยึดที่เดิมเสมอ
     * ไม่มีตำแหน่งประธานชมรม บรรทัดชื่อจะเลื่อนลงมาชิดบรรทัดชื่อชมรม
     * ใส่ตำแหน่งภายหลังเมื่อใด ตำแหน่งจะกลับเข้าไปอยู่ระหว่างสองบรรทัดนั้นเอง
     */
    var gClubY = oy + 45.8;
    var gPosY = gClubY - 2.1;
    var gNameY = (sign.position ? gPosY : gClubY) - 2.3;

    /*
     * ขอบบนของช่องลายเซ็นคำนวณจากขอบล่างจริงของข้อความสมาชิก
     * จึงไม่มีทางทับกันได้ ไม่ว่าชื่อหน่วยงานจะยาวจนขึ้นสองบรรทัดหรือไม่
     * ถ้าที่ว่างเหลือน้อย ช่องลายเซ็นจะเตี้ยลง แต่ไม่ถอยขึ้นไปทับข้อความ
     */
    var gSigBottom = gNameY - 2.4;
    var gSigTop = Math.max(memberBottom + 0.6, gSigBottom - 6.4);
    var gSigH = Math.max(2.4, gSigBottom - gSigTop);

    if (!drawFit(d, sign.signature, gx, gSigTop, gw, gSigH)) {
      // ยังไม่ได้แนบลายเซ็นประธานชมรม เว้นเส้นไว้ให้ลงนามจริงบนบัตรที่พิมพ์แล้ว
      d.setDrawColor(COLOR.line[0], COLOR.line[1], COLOR.line[2]);
      d.setLineWidth(0.2);
      d.line(gx + 3, gSigBottom, gx + gw - 3, gSigBottom);
    }
    fitText(d, "(" + (sign.name || "...................................................") + ")", {
      x: gcx, y: gNameY, maxWidth: gw, size: 4.3, minSize: 3, align: "center", color: COLOR.ink
    });
    if (sign.position) {
      fitText(d, sign.position, {
        x: gcx, y: gPosY, maxWidth: gw, size: 3.8, minSize: 2.6, align: "center", color: COLOR.ink2
      });
    }
    fitText(d, "ประธาน" + (club.name || "ชมรมอนามัยสิ่งแวดล้อมจังหวัดบุรีรัมย์"), {
      x: gcx, y: gClubY, maxWidth: gw, size: 3.5, minSize: 2.4, align: "center", color: COLOR.brandDark
    });
  ...
  function drawBack(d, data, ox, oy) {
    ...
    rect(d, ox, oy, CARD_W, CARD_H, COLOR.white);
    rect(d, ox, oy, CARD_W, 6.9, COLOR.brandDark);
    fitText(d, club.name || "ชมรมอนามัยสิ่งแวดล้อมจังหวัดบุรีรัมย์", {
      x: ox + CARD_W / 2, y: oy + 4.5, maxWidth: CARD_W - 6,
      size: 6.4, minSize: 4.2, style: "bold", align: "center", color: COLOR.white
    });
    ...
    if (qrOk) {
      fitText(d, "สแกนเพื่อตรวจสอบสมาชิก", {
        x: qx + qs / 2, y: qy + qs + 2.8, maxWidth: qs + 6,
        size: 4.3, minSize: 3.2, align: "center", color: COLOR.ink3
      });
    } else {
    ...
    fitText(d, "ที่อยู่ชมรม", {
      x: lx, y: oy + 11.4, maxWidth: lw, size: 5.1, minSize: 3.6, style: "bold", color: COLOR.brandDark
    });

    // ที่อยู่ชมรม: ย่อขนาดและตัดบรรทัดให้พอดีบัตร ไม่ตัดข้อความ
    var addr = fitText(d, club.address || "", {
      x: lx, y: oy + 15.2, maxWidth: lw, maxLines: 4,
      size: 5.0, minSize: 3.1, lineFactor: 1.16, color: COLOR.ink2
    });
    ...
    if (contact.length) {
      fitText(d, contact.join("  ·  "), {
        ...
        size: 4.6, minSize: 3.2, lineFactor: 1.14, color: COLOR.ink2
      });
      yc += 4.8;
    }
    ...
    fitText(d, "เลขที่บัตร " + (card.card_no || "-"), {
      x: lx, y: Math.min(yc + 1.8, oy + 37.6), maxWidth: lw,
      size: 4.8, minSize: 3.4, style: "bold", color: COLOR.ink
    });
    fitText(d, "รหัสตรวจสอบ " + (card.verify_token || "-"), {
      x: lx, y: Math.min(yc + 5.8, oy + 41.6), maxWidth: lw,
      size: 3.9, minSize: 2.8, color: COLOR.ink3
    });
    ...
    fitText(d,
      ...
      {
        x: ox + 4.4, y: oy + CARD_H - 7.4, maxWidth: CARD_W - 8.8, maxLines: 3,
        size: 4.0, minSize: 2.8, lineFactor: 1.18, color: COLOR.ink3
      }
    );
